...
#     params[f"W{plev}"] =  {"folder_path":f"{meteo_folder}/erainterim/plev","file_prefix":"P","netCDF_name":"W",
#                            "size":[41,95],"hourly_res":6,"title":f"Vertical Wind at {plev}hPa [m/s]"}
    params[f"OMEGA{plev}"] =  {"folder_path":f"{meteo_folder}/era5/plev","file_prefix":"P","netCDF_name":"OMEGA",
                           "size":[81,189],"hourly_res":3,"title":f"Vertical Velocity: Lagrangian Tendency of Air Pressure at {plev}hPa [Pa/s]"}
...
handler = MeteorologyToPandasHandler_Super(
    params, dates=None, debug=False, keep_na=False, result_size=[81,189],result_hourly_res=3
)
...
lats = list(range(200,281)) # 10 to 50
lons = list(range(296,485)) # -32 to 62
params_basenames = ['Z','U','V','OMEGA','Q','T','PV']
for param_basename in params_basenames:
...
# handler.create_yearly_dataframes_parallel_batches(base_filename,years=years,add_year_to_name=True,njobs=8,
#                                                   batch_size=30)


# Fixing missing batches due to format error
years = list(range(2000,2008))
base_filename = "../../data/meteorology_dataframes_62_81_189/meteorology_df"
handler.create_yearly_dataframes_parallel_batches(base_filename,years=years,add_year_to_name=True,njobs=8,
                                                  batch_size=30,num_batch_start=68,num_batch_end=82)


# Adding last batch (97) fix
years = list(range(2000,2021))
base_filename = "../../data/meteorology_dataframes_62_81_189/meteorology_df"
handler.create_yearly_dataframes_parallel_batches(base_filename,years=years,add_year_to_name=True,njobs=10,
                                                  batch_size=30,num_batch_start=97,num_batch_end=None)
...
# After fixing interpolation bug
years = list(range(2000,2021))
base_filename = "../../data/meteorology_dataframes_62_81_189/meteorology_df"
handler.create_yearly_dataframes_parallel_batches(base_filename,years=years,add_year_to_name=True,njobs=11,
                                                  batch_size=30,num_batch_start=64,num_batch_end=None)
...
df = torch.load(f"{base_filename}_2003_b0.pkl")


df["W900"]


t = torch.load("../../data/meteorology_tensors_62_81_189/meteorology_tensor_62_81_189_2000_b0_tensor.pkl")
...
sample_df = torch.load(f'{base_filename_debug}_2000.pkl')


sample_df[:5]


sample_df["pm10"][0].shape
...
for lev in clouds_model_levs:
    print(sample_df[f"CIWC{lev}"][200][20,30])
print(sample_df["CIWC_avg"][200][20,30])


sample_df["CRWC_avg"][0].shape
