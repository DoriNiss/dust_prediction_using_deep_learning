...
dust_df_metadata["PM25"][0]


base_stations_list = list(set([s['Name'] for s in dust_df_metadata["PM25"] if 'Name' in s.keys()]+
                              [s['Name'] for s in dust_df_metadata["PM10"] if 'Name' in s.keys()]))
len(base_stations_list)


base_stations_filename = f"{dust_dir}/base_stations.pkl"
torch.save(base_stations_list,base_stations_filename)
...
dust_csv = load_csv_file(dust_filename)

for i in range(100):
    print(i,dust_csv[i][0])
